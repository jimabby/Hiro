...
- **A cancelled scan is recorded as one** — stopping a scan honours the request by *returning* rather than throwing, which meant the run finished unwinding a moment later, saw no error, and filed itself as a clean success: two green "Scan complete" toasts for a scan you had just stopped, a `lastScanAt` claiming a sweep that never happened, and a webhook telling Discord the same. Cancelling is now its own outcome, distinct from both success and failure — it says so on the dashboard with an offer to run it again, and it announces nothing to your phone, your webhooks or the OS, because you are standing right there
- **Health per career board, not just per platform** — boards are watched one employer at a time and fail one at a time: a slug that gets renamed or made private returns 404 while the other five keep working. That failure used to be discarded unless *every* board failed, so a board could be dead for weeks while each morning's scan reported success and the aggregate health row stayed green, because the category really was working. Each board now has its own row, and its own vocabulary — a 404 is called out immediately and names the slug to fix, since unlike an empty scrape it is unambiguous and will not fix itself, while an employer with nothing open is never dressed up as a fault. The count reported is what the board published rather than what survived your keyword filter, so a narrow search does not read as a broken board
...
- **Getting started checklist** — a fresh install used to land on an empty table and a "Run Scan Now" button that, pressed with no resume, no AI key or no board login, ended in an error toast. Until the first application exists, the Dashboard now shows the four things a scan actually needs — resume, AI provider, a logged-in board or watched career board, and a Test Scan — each ticked from the same config and session state the scan itself checks, with a button to the place that fixes it. It goes away on its own once there is history
- **Search** — live search by job title or company (`/` to focus)
- **Keyboard navigation** — `1`–`9` switch pages in sidebar order, `↑`/`↓` move between rows, `Escape` closes the detail panel, and `?` (or the button in the sidebar footer) lists every shortcut. None of them fire while you are typing in a field or a dialog is open
- **Copy the documents** — the cover letter and tailored resume in the detail panel have a Copy button beside View and Save, because the destination is usually someone else's web form
...
- **Deletes can be taken back** — removing an application takes its tailored resume, cover letter, recruiter replies and interview history with it, and it is one click in a dense row of controls. Every delete now offers an Undo for as long as the toast is up, restoring the row and everything that hung off it under the same id. "Delete selected" and "Clear all" coalesce into a single undo rather than leaving only the last row recoverable — an undo button that silently restores one of ten is worse than none. Backups remain the answer for anything older than the last click
...
- **On your phone, and it will tell you before it expires** — `respond_by` is the only date in Hiro that somebody else set and that expires whether or not you look at it. Every other date the apps track is one Hiro chose, or is advisory. So the board is mirrored to the phone read-only, and a notification goes out seven days from the deadline, again at three, and again on the last day. Accepting, declining and the negotiation draft stay on the desktop — those are not decisions to take one-handed on a train. The money and your own notes travel inside the same encrypted envelope as everything else identifying: an advertised band is what an employer published, but a base and bonus are what somebody is privately willing to pay *you*
