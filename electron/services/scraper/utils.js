...
async function buildAnalyticsReportPDF(data) {
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(20).font('Helvetica-Bold').fillColor(NAVY)
      .text('Weekly Application Report', ML, ML, { align: 'center', width: CW })
    doc.moveDown(0.2)
    doc.fontSize(11).font('Helvetica').fillColor(GREY)
      .text(`${data.dateFrom} — ${data.dateTo}`, { align: 'center', width: CW })
    doc.moveDown(0.5)
    doc.moveTo(ML, doc.y).lineTo(ML + CW, doc.y).strokeColor(BLUE).lineWidth(1.5).stroke()
    doc.moveDown(0.8)

    // Summary stats
    doc.fontSize(13).font('Helvetica-Bold').fillColor(NAVY).text('Summary', ML)
    doc.moveDown(0.3)
    const stats = [
    ...
    // By platform
    if (data.byPlatform && Object.keys(data.byPlatform).length) {
      doc.fontSize(13).font('Helvetica-Bold').fillColor(NAVY).text('By Platform', ML)
      doc.moveDown(0.3)
      for (const [platform, count] of Object.entries(data.byPlatform)) {
        doc.fontSize(10).font('Helvetica').fillColor(BODY)
          .text(`•   ${platform}: ${count}`, ML + 10, doc.y)
      }
      doc.moveDown(0.6)
    }
    ...
    if (data.byStatus && Object.keys(data.byStatus).length) {
      doc.fontSize(13).font('Helvetica-Bold').fillColor(NAVY).text('By Status', ML)
      doc.moveDown(0.3)
      for (const [status, count] of Object.entries(data.byStatus)) {
        doc.fontSize(10).font('Helvetica').fillColor(BODY)
          .text(`•   ${status}: ${count}`, ML + 10, doc.y)
      }
      doc.moveDown(0.6)
    }

    // Top companies
    if (data.topCompanies && data.topCompanies.length) {
      doc.fontSize(13).font('Helvetica-Bold').fillColor(NAVY).text('Top Companies', ML)
      doc.moveDown(0.3)
      for (const c of data.topCompanies) {
        doc.fontSize(10).font('Helvetica').fillColor(BODY)
          .text(`•   ${c.company} (${c.count})`, ML + 10, doc.y)
      }
      doc.moveDown(0.6)
    }

    // Daily breakdown
    if (data.perDay && data.perDay.length) {
      doc.fontSize(13).font('Helvetica-Bold').fillColor(NAVY).text('Daily Breakdown', ML)
      doc.moveDown(0.3)
      for (const d of data.perDay) {
        const bar = '█'.repeat(Math.min(d.count, 30))
        doc.fontSize(9).font('Helvetica').fillColor(BODY)
          .text(`${d.date}  `, ML + 10, doc.y, { continued: true })
        doc.fillColor(BLUE).text(`${bar} `, { continued: true })
        doc.fillColor(GREY).text(`${d.count}`)
      }
    }
    ...
    doc.fontSize(8).font('Helvetica').fillColor(GREY)
      .text('Generated by Hiro — AI Job Application Assistant', ML, doc.y, { align: 'center', width: CW })
